...
class MolmoProcessor(ProcessorMixin):
    ...
    def __call__(self, text: list[str], images: list[Image], return_tensors: str = "pt", **kwargs):
        if return_tensors != "pt":
            warnings.warn("Only return_tensors='pt' is supported for MolmoProcessor")
        if len(text) > 1:
            raise ValueError("Only batch size of 1 is supported for MolmoProcessor with only one image per text")

        processed = self.process(text=text[0], images=images, **kwargs)
