...
class MolmoBlock(nn.Module):
    ...
    def _scaled_dot_product_attention(
        ...
        if self.flash_attn_func is not None and attn_mask is None:
            ...
            sdpa = torch.matmul(attn_weights, v)
            # with open("/home/projects/shimon/agroskin/projects/vlmflow/debug/sdpa_arr.pkl", 'rb') as f:
            #     sdpa_arr = pickle.load(f)
            # sdpa_arr.append(sdpa)
            # with open("/home/projects/shimon/agroskin/projects/vlmflow/debug/sdpa_arr.pkl", 'wb') as f:
            #     pickle.dump(sdpa_arr, f)
            return sdpa, attn_weights
...
class MolmoSequentialBlock(MolmoBlock):
    ...
    def forward(
        ...
        if self._activation_checkpoint_fn is not None:
            ...
            att, cache, attn_weights = self.attention(q, k, v, attention_bias, position_ids=position_ids, layer_past=layer_past, use_cache=use_cache)
    ...
    def forward(
        ...
        if self.config.block_group_size == 1:
            for block_idx, (block, attention_bias) in enumerate(zip(self.transformer.blocks, attention_biases)):
                ...
                x, cache, attn_weights = block(x, attention_bias=attention_bias, position_ids=position_ids, layer_past=layer_past, use_cache=use_cache)
        ...
        return ModelOutput(logits=logits, attn_key_values=attn_key_values, hidden_states=tuple(all_hidden_states) if output_hidden_states else None,
                           attentions=tuple(all_attentions) if output_attentions else None)  # type: ignore[arg-type]
