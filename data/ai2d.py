# AI2D Preprocessing code taken from the Molmo repo:
# https://github.com/allenai/molmo/blob/main/olmo/hf_datasets/ai2d.py#L92
...
from data.base import EvalWrapper, BaseDataset
...
class AI2D(BaseDataset):
    table_columns = ["idx", "image", "question", "prediction", "answer"]

    def __init__(self):
        self.dataset = Ai2dDatasetBuilder().as_dataset('test', in_memory=True)
        self.dataset = self.dataset.filter(lambda x: not x["abc_label"] or x["has_transparent_box"],
                                           keep_in_memory=True)
    ...
    def __getitem__(self, idx):
        ...
        options = sample["answer_texts"]
        options_are_abc = sample["abc_label"] and sum(sample["option_is_abc"]) == len(options)
        if options_are_abc:
            options = [
                opt.upper() if abc else opt
                for opt, abc in zip(options, sample["option_is_abc"])
            ]
        options = [re.split(r'\([ABCD]\)', opt)[0].strip() for opt in options]  # Cleaning garbage labels
        answer = sample["correct_answer"]
        return idx, image, question, options, answer, options_are_abc

    def table_repr(self, idx, prediction, img_as_object=True):
        sample = self.dataset[idx]
        options = sample["answer_texts"]
        options_are_abc = sample["abc_label"] and sum(sample["option_is_abc"]) >= (len(options) - 1)
        if options_are_abc:
            options = [
                opt.upper() if abc else opt
                for opt, abc in zip(options, sample["option_is_abc"])
        ...
        return [
            idx,
            wandb.Image(sample['image']) if img_as_object else sample['image_name'],
            sample["question"],
            f"{chr(prediction + ord('A'))}: {options[prediction]}",
            f"{sample['correct_answer']}: {options[sample['correct_answer']]}",
        ]
...
class AI2DCollate:
    def __call__(self, batch):
        ...
        for b in batch:
            idx.append(b[0])
            images.append(b[1])
            questions.append(b[2])
            choices.append(b[3])
            answers.append(b[4])
            options_are_abc.append(b[5])
        idx = torch.tensor(idx)
        return torch.LongTensor(idx), images, questions, choices, torch.LongTensor(answers), options_are_abc


class AI2DEval(EvalWrapper):
    ...
    def __call__(self, batch, model) -> 'EvaluationResult':
        idx, images, questions, choices, answers, options_are_abc = batch
        batch_size = idx.shape[0]
        # the score_single_tokens method cannot deal with different vqa candidates for different samples in a batch
        assert batch_size == 1, f"Batch size > 1 is not supported"
        if options_are_abc[0]:
            text = f"{self.pre_prompt}{questions[0]}{self.mid_prompt}Choices: {", ".join(choices[0])}\n{self.post_prompt}"

            vqa_candidates = [f" {opt.strip()}" for opt in choices[0]]  # Only supports the Molmo prompt format for now
        else:
            text = f"{self.pre_prompt}{questions[0]}{self.mid_prompt}"
            for i, cand in enumerate(choices[0]):
                text += f"{chr(i + ord('A'))}. {cand}\n"
            text += self.post_prompt
            vqa_candidates = model.vqa_candidates

        texts = [text]
        match self.eval_method:
            case "abcd":
                scores, generated_ids, num_generated_tokens = model.score_single_tokens(images, texts, vqa_candidates)
                predictions = scores.argmax(-1).cpu()

            case "blind":
                scores, generated_ids, num_generated_tokens = model.score_single_tokens(None,  # Here!
                                                                                        texts,
                                                                                        vqa_candidates)
                predictions = scores.argmax(-1).cpu()
            case _:
